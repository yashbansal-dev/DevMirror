import { ProjectManifest, EnvironmentVariable, Service, Database } from '@devmirror/shared';
...
export class ProjectAnalyzer {
  ...
  async analyze(repoPathOrUrl: string): Promise<{ projectDir: string; manifest: ProjectManifest; isTemporary: boolean }> {
    ...
    if (projectName === '.' || !projectName) projectName = 'app';
    ...
    for (const ep of detection.entrypoints) {
      if (ep.port && !ports.includes(ep.port)) ports.push(ep.port);
    }
    for (const svc of detection.services) {
      if (svc.port && !ports.includes(svc.port)) ports.push(svc.port);
    }
  ...
  private async resolveRepository(repoPathOrUrl: string): Promise<{ projectDir: string; isTemporary: boolean }> {
  ...
  private async extractEnvFiles(projectDir: string): Promise<EnvironmentVariable[]> {
