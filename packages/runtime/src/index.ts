import { ProjectManifest, HealthCheck } from '@devmirror/shared';
...
export class ProjectRunner {
  ...
  private activeProcess?: any;

  getLogs(): ExecutionLog[] {
    return [...this.logs];
  }

  private log(source: ExecutionLog['source'], type: ExecutionLog['type'], message: string) {
  ...
  async installDependencies(projectDir: string, manifest: ProjectManifest): Promise<boolean> {
    ...
    try {
      ...
      if (stdout) this.log('install', 'stdout', stdout);
      ...
      if (err.stdout) this.log('install', 'stdout', err.stdout);
      if (err.stderr) this.log('install', 'stderr', err.stderr);
      return false;
    }
  }
  ...
  async runProject(
  ...
  ): Promise<{ success: boolean; accessUrl: string; logs: ExecutionLog[] }> {
    ...
    try {
      ...
      this.activeProcess.all?.on('data', (chunk: any) => {
  ...
  async checkHealth(targetUrl: string, maxAttempts = 5): Promise<boolean> {
