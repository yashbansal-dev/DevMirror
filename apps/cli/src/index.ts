  ...
  .action(async (repository = '.', options) => {
    ...
    try {
      ...
      if (options.json) {
        ...
        if (manifest.packageManager) console.log(chalk.gray(`✓ Package Manager:    ${manifest.packageManager}`));
  ...
  .action(async (repository = '.', options) => {
    ...
    try {
      ...
      table.push(
        ...
        ['Framework', manifest.framework || 'None / Custom'],
        ['Package Manager', manifest.packageManager || 'None'],
        ['Runtime', manifest.runtime || 'Default'],
        ['Docker Compose', manifest.hasDockerCompose ? 'Yes' : 'No'],
        ['Services', manifest.services.map(s => s.name).join(', ') || 'None'],
        ['Databases', manifest.databases.map(d => d.type).join(', ') || 'None'],
        ['Environment Vars', `${manifest.environmentVariables.length} variables detected`],
        ['Start Command', manifest.commands.dev]
      );
