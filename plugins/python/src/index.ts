...
export class PythonPlugin implements DevMirrorPlugin {
  name = 'python';
  ...
  async detect(projectDir: string, files: string[]): Promise<DetectionResult> {
    ...
    if (files.includes('poetry.lock') || lower.includes('[tool.poetry]')) pm = 'poetry';
    else if (files.includes('uv.lock') || lower.includes('[tool.uv]')) pm = 'uv';
    else if (files.includes('Pipfile') || files.includes('Pipfile.lock')) pm = 'pipenv';
    ...
    if (pm === 'poetry') installCmd = 'poetry install';
    else if (pm === 'uv') installCmd = 'uv sync';
    else if (pm === 'pipenv') installCmd = 'pipenv install';
